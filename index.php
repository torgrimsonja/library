		...
		if($checkForStudent->num_rows){
			...
			if($checkoutValidation->num_rows){
				...
				if(!$db->query('SELECT `'.$sql['block'].'` FROM student WHERE id = '.$sql['id'], 'teacherEmailAddress')){
					...
					$sql['teacherName'] = $db->escape_string($teacherEmailAddress->fetch_assoc()$sql['block']);
					//query for alternate email address
					$alternateEmail = $db->query('SELECT emailAddress FROM alternate_email_address WHERE name = \''.$sql['teacherName'].'\'', 'alternateEmail');
					if($alternateEmail->num_rows){
						$html['to'] = $data_validation->escape_html($alternateEmail->fetch_assoc()['emailAddress'];
					}else{
						$tmpArray = explode(',', $sql['teacherName']);
						$lastname = trim($tmpArray[0]);
						$firstname = substr(trim($tmpArray[1]), 0, 2);
